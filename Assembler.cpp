...
: nLines(text->nLines), labels(new Label[text->nLines]), nLabels(0), lines(text->lines),
  assembly(new char[nLines * std::max(sizeof(char) + sizeof(char) + sizeof(char) + sizeof(Imm), sizeof(Imm))]),
...
void Assembler::translateAddress(Imm address)
{
    *(Imm *) translator = address;
    translator += sizeof(Imm);
}
...
bool Assembler::isReg(const char *arg)
{
    ...
    return (arg[0] == 'r') && ((arg[1] >= 'a') && (arg[1] <= 'h')) && (arg[2] == 'x');
}
...
{
    readWriteMode.imm = 1;
    std::fprintf(listingFile, "0x%08X 0x%08dX ", '0' + readWriteMode.mode, std::atoi(cmdArg));
...
void Assembler::translateReadWriteInstructionRegWithOffset(char *cmdArg, ReadWriteMode readWriteMode, char regCode)
{
    ...
    *plus = '\0';
    ...
    std::fprintf(listingFile, "0x%08X 0x%08X 0x%08dX ", '0' + readWriteMode.mode, '0' + regCode, std::atoi(constant));
...
void Assembler::pass(unsigned passNo)
{
    ...
    for (size_t i = 0; i < nLines; ++i) {
        ...
        std::fprintf(listingFile, "0x%08dX ", translator - assembly);
